...
// Version 3 created on 04JAN23 at 1902
...
function draw() {
  ...
  } else if (floor(frameCount/90) <= 40) {
  ...
  } else if (floor(frameCount/90) <= 60) {
...
class Timer {
  ...
  show(){
    ...
    } else if (floor(frameCount/90) >= 50) {
      // Runs this time code 10 seconds before time is up
      textSize(width * (3 / 80));
      
      fill(255, 0, 0);
      text("Time: " + String(60 - floor(frameCount/90)), width/40, height/20);
...
class Cursor {
  ...
  // Checks for collsion on regular meteors
  checkForMeteorCollision(meteor){
  ...
  checkForFireMeteorCollision(fireMeteor){
  ...
  // Checks for collision on ice meteors
  checkForIceMeteorCollision(iceMeteor){
...
class FireMeteor extends Meteor {
...
class IceMeteor extends Meteor {
